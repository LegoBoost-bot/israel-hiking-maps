...
export class LocalVectorTileCacheService {
    ...
    public createRectangleRegion(tileKeys: string[], label?: string): LocalVectorTileCacheRegion {
        return {
            id: uuidv4(),
            source: "mapTile",
    ...
    private async getDownloadTasksForRegion(region: LocalVectorTileCacheRegion): Promise<(ParsedTileUrl & { regionTileKeys: string[] })[]> {
        ...
        const chunkSize = 500;
        for (let i = 0; i < region.tileKeys.length; i += chunkSize) {
            const chunk = region.tileKeys.slice(i, i + chunkSize);
            for (const tileKey of chunk) {
                const { tileX, tileY } = this.parseTileKey(tileKey);
