...
import { RouteData, LatLngAltTime, ApplicationState, RouteEditStateType } from "../models";
...
export class PrintService {
    ...
    public async export(
        ...
        const originalRouteStates = new Map<string, RouteEditStateType>();
        const allRoutes = this.store.selectSnapshot((s: ApplicationState) => s.routes.present);

        const hillshadeLayerIds: { id: string; visibility: unknown }[] = [];
        if (!includeHillshade) {
        ...
        if (route || mode === "all") {
            for (const [id, state] of originalRouteStates) {
                this.store.dispatch(new ChangeRouteStateAction(id, state));
            }
        }

        for (const hillshadeLayer of hillshadeLayerIds) {
            map.setLayoutProperty(hillshadeLayer.id, "visibility", hillshadeLayer.visibility as "visible" | "none");
        }
    }
    ...
    private getLatlngs(routeData: { segments?: readonly { latlngs?: readonly LatLngAltTime[] }[]; markers?: readonly { latlng?: LatLngAltTime }[] }): LatLngAltTime[] {
        let latLngs: LatLngAltTime[] = [];
        for (const segment of routeData.segments ?? []) {
            if (segment.latlngs) {
                latLngs = latLngs.concat(segment.latlngs as LatLngAltTime[]);
            }
        }
    ...
    private async waitForIdle(map: MapLibreMap): Promise<void> {
        return new Promise<void>(resolve => {
            if (typeof (map as unknown as { isIdle?: () => boolean }).isIdle === "function" && (map as unknown as { isIdle: () => boolean }).isIdle()) {
                resolve();
            } else {
